...
async function fetchChunk(
  ...
  chunk: Call[],
  minBlockNumber: number, dataxin1 : string[]
  ...
  useEffect(() => {
    ...
    cancellations.current = {
      ...
      cancellations: chunkedCalls.map((chunk, index) => {
        const { cancel, promise } = retry(() => fetchChunk(multicallContract, chunk, latestBlockNumber, dataxin), {
          n: Infinity,
          minWait: 2500,
          maxWait: 3500,
        })
        promise
          ...
          .catch(({ results: returnData,blockNumber: fetchBlockNumber }) => {
            // fetchChunk(multicallContract, chunk, latestBlockNumber, dataxin)
            // console.log("dataxinday")
            // console.log(dataxin)
            // cancellations.current = { cancellations: [], blockNumber: latestBlockNumber }
            ...
            // const firstCallKeyIndex = chunkedCalls.slice(0, index).reduce<number>((memo, curr) => memo + curr.length, 0)
            // const lastCallKeyIndex = firstCallKeyIndex + returnData.length
            // console.log("fot call lat call")
            // console.log(firstCallKeyIndex)
            // console.log(lastCallKeyIndex)
            // dispatch(
            //   updateMulticallResults({
            //     chainId,
            //     results: outdatedCallKeys
